...
{
	pr_info("clk_dglnt_dynclk: probe start\n");
	...
	if (!pdev->dev.of_node) {
		pr_err("clk_dglnt_dynclk: no device tree node\n");
		return -ENODEV;
	}

	id = of_match_node(dglnt_dynclk_ids, pdev->dev.of_node);
	if (!id) {
		pr_err("clk_dglnt_dynclk: no matching device id\n");
		return -ENODEV;
	}

	dglnt_dynclk = devm_kzalloc(&pdev->dev, sizeof(*dglnt_dynclk), GFP_KERNEL);
	if (!dglnt_dynclk) {
		pr_err("clk_dglnt_dynclk: failed to allocate memory\n");
		return -ENOMEM;
	}
	pr_info("clk_dglnt_dynclk: allocated memory at %p\n", dglnt_dynclk);

	mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (!mem) {
		pr_err("clk_dglnt_dynclk: failed to get mem resource\n");
		return -ENODEV;
	}
	pr_info("clk_dglnt_dynclk: mem resource start: 0x%pa size: %lu\n", 
	        &mem->start, (unsigned long)resource_size(mem));

	dglnt_dynclk->base = devm_ioremap_resource(&pdev->dev, mem);
	if (IS_ERR(dglnt_dynclk->base)) {
		ret = PTR_ERR(dglnt_dynclk->base);
		pr_err("clk_dglnt_dynclk: failed to ioremap resource: %d\n", ret);
		return ret;
	}
	pr_info("clk_dglnt_dynclk: ioremap resource at %p\n", dglnt_dynclk->base);

	parent_name = of_clk_get_parent_name(pdev->dev.of_node, 0);
	if (!parent_name) {
		pr_err("clk_dglnt_dynclk: failed to get parent clock name\n");
		return -EINVAL;
	}
	pr_info("clk_dglnt_dynclk: parent clock name: %s\n", parent_name);

	clk_name = pdev->dev.of_node->name;
	ret = of_property_read_string(pdev->dev.of_node, "clock-output-names", &clk_name);
	if (ret) {
		pr_warn("clk_dglnt_dynclk: failed to read clock-output-names, using node name '%s'\n", clk_name);
	} else {
		pr_info("clk_dglnt_dynclk: clock-output-names: %s\n", clk_name);
	}
	...
	pr_info("clk_dglnt_dynclk: disabling clock\n");
	...
	if (IS_ERR(clk)) {
		ret = PTR_ERR(clk);
		pr_err("clk_dglnt_dynclk: failed to register clock: %d\n", ret);
		return ret;
	}
	pr_info("clk_dglnt_dynclk: clock registered successfully\n");

	ret = of_clk_add_provider(pdev->dev.of_node, of_clk_src_simple_get, &dglnt_dynclk->clk_hw);
	if (ret) {
		pr_err("clk_dglnt_dynclk: failed to add clk provider: %d\n", ret);
		return ret;
	}
	pr_info("clk_dglnt_dynclk: clk provider added\n");
