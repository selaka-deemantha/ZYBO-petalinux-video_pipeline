 ...
 * Debugging additions by ChatGPT
 */
...
static int digilent_hdmi_get_modes(struct drm_connector *connector)
{
	...
	pr_info("[digilenthdmi] hdmi_get_modes called\n");

	if (hdmi->i2c_bus) {
		pr_info("[digilenthdmi] i2c bus valid\n");

		edid = drm_get_edid(connector, hdmi->i2c_bus);
		if (!edid) {
			dev_err(hdmi->dev, "[digilenthdmi] failed to get edid data\n");
			return 0;
		}
		...
		pr_info("[digilenthdmi] edid data retrieved\n");
		pr_info("[digilenthdmi] mode count: %d\n", count);
	} else {
		pr_info("[digilenthdmi] no i2c bus — using fallback modes\n");
		count = drm_add_modes_noedid(connector, hdmi->hmax, hdmi->vmax);
		drm_set_preferred_mode(connector, hdmi->hpref, hdmi->vpref);
		pr_info("[digilenthdmi] fallback mode count: %d\n", count);
	}
...
static enum drm_connector_status digilent_hdmi_detect(struct drm_connector *connector, bool force)
{
	...
	if (!hdmi->i2c_bus) {
		pr_info("[digilenthdmi] detect: no i2c bus\n");
		return connector_status_unknown;
	}

	bool connected = drm_probe_ddc(hdmi->i2c_bus);
	pr_info("[digilenthdmi] detect: %s\n", connected ? "connected" : "disconnected");
...
static int digilent_hdmi_create_connector(struct digilent_hdmi *hdmi)
{
	...
	ret = drm_connector_init(hdmi->drm_dev, connector, &digilent_hdmi_connector_funcs,
	...
	pr_info("[digilenthdmi] connector created\n");
	return 0;
}
                                          ...
                                          struct drm_connector_state *connector_state)
{
	...
	pr_info("[axidynclk] set_rate entered, rate: %d kHz\n", m->clock);
	clk_set_rate(hdmi->clk, m->clock * 1000);
	pr_info("[axidynclk] set_rate succeeded, frequency set to: %lu Hz\n", clk_get_rate(hdmi->clk));
}
...
static void digilent_hdmi_enable(struct drm_encoder *encoder)
{
	...
	pr_info("[digilenthdmi] encoder enabled\n");
}
...
static void digilent_hdmi_disable(struct drm_encoder *encoder)
{
	...
	pr_info("[digilenthdmi] encoder disabled\n");
}
...
static int digilent_hdmi_create_encoder(struct digilent_hdmi *hdmi)
{
	...
	pr_info("[digilenthdmi] encoder created\n");
	return 0;
}
...
static int digilent_hdmi_parse_dt(struct digilent_hdmi *hdmi)
{
	...
	if (IS_ERR(hdmi->clk)) {
		...
		dev_err(dev, "[digilenthdmi] failed to get hdmi clock: %d\n", ret);
		return ret;
	}
	...
	if (i2c_node) {
		...
		if (!hdmi->i2c_bus) {
			ret = -EPROBE_DEFER;
			dev_err(dev, "[digilenthdmi] failed to get edid i2c adapter: %d\n", ret);
			return ret;
		}
	} else {
		dev_info(dev, "[digilenthdmi] edid i2c property not found\n");
	}

	if (of_property_read_u32(node, "digilent,fmax", &hdmi->fmax))
		hdmi->fmax = DIGILENT_ENC_MAX_FREQ;
	if (of_property_read_u32(node, "digilent,hmax", &hdmi->hmax))
		hdmi->hmax = DIGILENT_ENC_MAX_H;
	if (of_property_read_u32(node, "digilent,vmax", &hdmi->vmax))
		hdmi->vmax = DIGILENT_ENC_MAX_V;
	if (of_property_read_u32(node, "digilent,hpref", &hdmi->hpref))
		hdmi->hpref = DIGILENT_ENC_PREF_H;
	if (of_property_read_u32(node, "digilent,vpref", &hdmi->vpref))
		hdmi->vpref = DIGILENT_ENC_PREF_V;

	pr_info("[digilenthdmi] device tree parsed successfully\n");
	return 0;
}
...
static int digilent_hdmi_bind(struct device *dev, struct device *master, void *data)
{
	...
	pr_info("[digilenthdmi] bind complete\n");
	return 0;
}
...
static int digilent_hdmi_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(dev, "[digilenthdmi] failed to parse device tree: %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		dev_err(dev, "[digilenthdmi] fail to add component: %d\n", ret);
		return ret;
	}

	pr_info("[digilenthdmi] hdmi driver probed successfully\n");
	return 0;
}
...
static int digilent_hdmi_remove(struct platform_device *pdev)
{
	...
	pr_info("[digilenthdmi] driver removed\n");
	return 0;
}
